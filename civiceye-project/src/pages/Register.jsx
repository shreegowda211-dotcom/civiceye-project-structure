      ...
      <div className="grid w-full max-w-5xl overflow-hidden rounded-2xl bg-white shadow-lg ring-1 ring-slate-200 md:grid-cols-2">
        ...
        <div className="flex flex-col justify-center px-6 py-8 sm:px-8 md:px-10">
          <div className="mx-auto w-full max-w-sm">
            <GradientCard title="Create Account" description="Register for CivicEye platform">
              {message && (
              ...
              <div className="mt-6">
                <label className="block text-xs font-medium text-white">Select Your Role</label>
                <div className="mt-3 grid grid-cols-1 justify-items-center gap-4">
                  <button type="button" onClick={() => setRole('Citizen')} aria-pressed={role === 'Citizen'} className={`group w-full sm:w-3/4 relative rounded-2xl border p-4 text-left transition-all duration-300 ${role === 'Citizen' ? 'border-emerald-500 bg-emerald-50 shadow-lg shadow-emerald-100 scale-[1.03]' : 'border-slate-200 bg-white hover:border-slate-300 hover:shadow-md'}`}>
                    <div className="flex items-center gap-3">
                      <div className={`rounded-xl p-2 transition ${role === 'Citizen' ? 'bg-emerald-500 text-black' : 'bg-slate-100 text-slate-600 group-hover:bg-slate-200'}`}>
              ...
              <button 
               type="submit" 
               disabled={loading} 
               className="mt-6 w-full rounded-md bg-white/20 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-white/30 disabled:opacity-50">
